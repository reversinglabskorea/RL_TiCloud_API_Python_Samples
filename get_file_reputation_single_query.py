...
if __name__ == '__main__':
    ...
    hash_type = 'sha1' # can be : sha1, md5, sha256

    # TCA0101 - File Reputation
    response = requests.get('%s/api/databrowser/malware_presence/query/%s/%s?format=%s' % (addr, hash_type, hash_code, result_format),
                           headers = hd)

    print('Response status code:', response.status_code)

    # Save the result > depends on a format type
    file_name = 'filereputation_%s.%s' % (hash_code, result_format)
